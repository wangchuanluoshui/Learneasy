package com.learneasy.orders.api;
...
import com.learneasy.orders.util.SecurityUtil;
...
public class OrderController {
    ...
    public PageResult<LeOrders> list(PageParams pageParams,@RequestBody LeOrders leOrders){
        return orderService.queryOrders(pageParams,leOrders);
    ...
    public void requestpay(String payNo, HttpServletResponse httpResponse) throws IOException {
        //如果支付记录号不存在则提示重新发起支付
        LePayRecord payRecord = orderService.getPayRecordByPayno(payNo);
        if(payRecord == null){
        ...
        if("601002".equals(status)){
            LearnEasyPlusException.cast("订单已支付，请勿重复支付。");
        }
        //构造sdk的客户端对象
...
//        alipayRequest.setNotifyUrl("http://tjxt-user-t.itheima.net/learnesy/orders/paynotify");//在公共参数中设置回跳和通知地址
        ...
        try {
            //请求支付宝下单接口,发起http请求
            form = client.pageExecute(alipayRequest).getBody(); //调用SDK请求支付宝下单
