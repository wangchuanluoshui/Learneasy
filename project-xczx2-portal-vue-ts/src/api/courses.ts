...
export async function commitCourse(courseBaseId: number): Promise<void> {
  await createAPI(`/content/courseaudit/commit/${courseBaseId}/202003`, 'post')
}

// 确认审核
export async function commitCourseD(courseBaseId: number): Promise<void> {
  await createAPI(`/content/courseaudit/commit/${courseBaseId}/202004`, 'post')
}
