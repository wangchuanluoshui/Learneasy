/*
 Navicat Premium Data Transfer

 Source Server         : 192.168
 Source Server Type    : MySQL
 Source Server Version : 80026
 Source Host           : localhost:3306
 Source Schema         : learneasy_users1

 Target Server Type    : MySQL
 Target Server Version : 80026
 File Encoding         : 65001

 Date: 21/09/2023 12:33:20
*/

SET NAMES utf8mb4;
SET FOREIGN_KEY_CHECKS = 0;

-- ----------------------------
-- Table structure for le_company
-- ----------------------------
DROP TABLE IF EXISTS `le_company`;
CREATE TABLE `le_company`  (
  `id` varchar(32) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL,
  `linkname` varchar(64) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '联系人名称',
  `name` varchar(128) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL COMMENT '名称',
  `mobile` varchar(11) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL,
  `email` varchar(128) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL,
  `intro` varchar(512) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '简介',
  `logo` varchar(128) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT 'logo',
  `identitypic` varchar(128) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '身份证照片',
  `worktype` varchar(32) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '工具性质',
  `businesspic` varchar(128) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '营业执照',
  `status` varchar(32) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '企业状态',
  PRIMARY KEY (`id`) USING BTREE
) ENGINE = InnoDB CHARACTER SET = utf8 COLLATE = utf8_general_ci ROW_FORMAT = DYNAMIC;

-- ----------------------------
-- Records of le_company
-- ----------------------------
INSERT INTO `le_company` VALUES ('1', '张老师', '传智播客', '13333334444', 'abc@126.com', '2006年创建！', NULL, NULL, NULL, NULL, NULL);
INSERT INTO `le_company` VALUES ('1232141425', 'M老师', '传智教育', '123456789', 'mm@qq.com', NULL, NULL, NULL, NULL, NULL, NULL);
INSERT INTO `le_company` VALUES ('2', '李老师', '博学谷', '', '', NULL, NULL, NULL, NULL, NULL, NULL);

-- ----------------------------
-- Table structure for le_company_user
-- ----------------------------
DROP TABLE IF EXISTS `le_company_user`;
CREATE TABLE `le_company_user`  (
  `id` varchar(32) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL,
  `company_id` varchar(32) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL,
  `user_id` varchar(32) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL,
  PRIMARY KEY (`id`) USING BTREE,
  UNIQUE INDEX `xc_company_user_unique`(`company_id` ASC, `user_id` ASC) USING BTREE,
  INDEX `FK_xc_company_user_user_id`(`user_id` ASC) USING BTREE,
  CONSTRAINT `FK_xc_company_user_company_id` FOREIGN KEY (`company_id`) REFERENCES `le_company` (`id`) ON DELETE RESTRICT ON UPDATE RESTRICT,
  CONSTRAINT `FK_xc_company_user_user_id` FOREIGN KEY (`user_id`) REFERENCES `le_user` (`id`) ON DELETE RESTRICT ON UPDATE RESTRICT
) ENGINE = InnoDB CHARACTER SET = utf8 COLLATE = utf8_general_ci ROW_FORMAT = DYNAMIC;

-- ----------------------------
-- Records of le_company_user
-- ----------------------------
INSERT INTO `le_company_user` VALUES ('1', '1', '49');
INSERT INTO `le_company_user` VALUES ('2', '1232141425', '52');

-- ----------------------------
-- Table structure for le_menu
-- ----------------------------
DROP TABLE IF EXISTS `le_menu`;
CREATE TABLE `le_menu`  (
  `id` varchar(64) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL,
  `code` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '菜单编码',
  `path` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '路由',
  `p_id` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '父菜单ID',
  `name` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '名称',
  `url` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '请求地址',
  `is_menu` char(1) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '是否是菜单',
  `level` int NULL DEFAULT NULL COMMENT '菜单层级',
  `sort` int NULL DEFAULT NULL COMMENT '菜单排序',
  `status` char(1) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,
  `icon` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '图标',
  `create_time` datetime NULL DEFAULT NULL,
  `update_time` datetime NULL DEFAULT NULL,
  `component` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '组件路径',
  `hidden` bit(1) NULL DEFAULT NULL COMMENT '是否隐藏',
  `redirect` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '重定向',
  `cache` bit(1) NULL DEFAULT NULL COMMENT '是否缓存',
  PRIMARY KEY (`id`) USING BTREE,
  UNIQUE INDEX `FK_CODE`(`code` ASC) USING BTREE
) ENGINE = InnoDB CHARACTER SET = utf8 COLLATE = utf8_general_ci ROW_FORMAT = DYNAMIC;

-- ----------------------------
-- Records of le_menu
-- ----------------------------
INSERT INTO `le_menu` VALUES ('000000000000000000', 'root', '/', '', '系统根目录', NULL, '0', 0, 1, '1', NULL, '2023-08-25 08:35:25', '2023-08-25 08:35:25', NULL, NULL, NULL, NULL);
INSERT INTO `le_menu` VALUES ('1', 'homepage', '/homepage', NULL, '网站首页', NULL, '1', 1, 1, '1', NULL, '2023-09-16 19:47:55', '2023-09-16 19:47:57', NULL, NULL, NULL, NULL);
INSERT INTO `le_menu` VALUES ('10', 'quanxian_1', '/yonghu', '9', '用户管理', NULL, '1', 2, 1, '1', NULL, NULL, NULL, NULL, NULL, NULL, NULL);
INSERT INTO `le_menu` VALUES ('11', 'quanxian_2', '/jueshe', '9', '角色管理', NULL, '1', 2, 1, '1', NULL, NULL, NULL, NULL, NULL, NULL, NULL);
INSERT INTO `le_menu` VALUES ('12', 'quanxian_3', '/menu', '9', '权限管理', NULL, '1', 2, 1, '1', NULL, NULL, NULL, NULL, NULL, NULL, NULL);
INSERT INTO `le_menu` VALUES ('13', 'quanxian_4', '/kechengshenhe', '9', '课程审核', NULL, '1', 2, 1, '1', NULL, NULL, NULL, NULL, NULL, NULL, NULL);
INSERT INTO `le_menu` VALUES ('2', 'homepage_1', '/home', '1', 'LearnEasy-C++首', NULL, '1', 2, 1, '1', NULL, NULL, NULL, NULL, NULL, NULL, NULL);
INSERT INTO `le_menu` VALUES ('3', 'jiaowu', '/jiaowu', '', '教务教学', NULL, '1', 1, 1, '1', NULL, NULL, NULL, NULL, NULL, NULL, NULL);
INSERT INTO `le_menu` VALUES ('4', 'jiaowu_1', '/kecheng', '3', '课程管理', NULL, '1', 2, 1, '1', NULL, NULL, NULL, NULL, NULL, NULL, NULL);
INSERT INTO `le_menu` VALUES ('5', 'jiaowu_2', '/meizi', '3', '媒资管理', NULL, '1', 2, 1, '1', NULL, NULL, NULL, NULL, NULL, NULL, NULL);
INSERT INTO `le_menu` VALUES ('6', 'jiaowu_3', '/zuoye', '3', '作业管理', NULL, '1', 2, 1, '1', NULL, NULL, NULL, NULL, NULL, NULL, NULL);
INSERT INTO `le_menu` VALUES ('7', 'caiwu', '/caiwu', '', '财务管理', NULL, '1', 1, 1, '1', NULL, NULL, NULL, NULL, NULL, NULL, NULL);
INSERT INTO `le_menu` VALUES ('8', 'caiwu_1', '/dingdan', '7', '财务管理', NULL, '1', 2, 1, '1', NULL, NULL, NULL, NULL, NULL, NULL, NULL);
INSERT INTO `le_menu` VALUES ('9', 'quanxian', '/quanxian', NULL, '用户管理', NULL, '1', 1, 1, '1', NULL, NULL, NULL, NULL, NULL, NULL, NULL);

-- ----------------------------
-- Table structure for le_permission
-- ----------------------------
DROP TABLE IF EXISTS `le_permission`;
CREATE TABLE `le_permission`  (
  `id` varchar(32) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL,
  `role_id` varchar(32) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL,
  `menu_id` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL,
  `create_time` datetime NULL DEFAULT NULL,
  PRIMARY KEY (`id`) USING BTREE,
  UNIQUE INDEX `xu_permission_unique`(`role_id` ASC, `menu_id` ASC) USING BTREE,
  INDEX `fk_xc_permission_menu_id`(`menu_id` ASC) USING BTREE
) ENGINE = InnoDB CHARACTER SET = utf8 COLLATE = utf8_general_ci ROW_FORMAT = DYNAMIC;

-- ----------------------------
-- Records of le_permission
-- ----------------------------
INSERT INTO `le_permission` VALUES ('1703577062264102914', '17', '1', NULL);
INSERT INTO `le_permission` VALUES ('1703577062276685826', '17', '2', NULL);
INSERT INTO `le_permission` VALUES ('1703577127108042753', '18', '1', NULL);
INSERT INTO `le_permission` VALUES ('1703577127116431362', '18', '2', NULL);
INSERT INTO `le_permission` VALUES ('1703577127124819969', '18', '3', NULL);
INSERT INTO `le_permission` VALUES ('1703577127133208578', '18', '4', NULL);
INSERT INTO `le_permission` VALUES ('1703577127141597185', '18', '5', NULL);
INSERT INTO `le_permission` VALUES ('1703577127149985793', '18', '6', NULL);
INSERT INTO `le_permission` VALUES ('1703577212315328513', '20', '1', NULL);
INSERT INTO `le_permission` VALUES ('1703577212323717121', '20', '2', NULL);
INSERT INTO `le_permission` VALUES ('1703577212487294978', '20', '13', NULL);
INSERT INTO `le_permission` VALUES ('1703577335548174337', '6', '7', NULL);
INSERT INTO `le_permission` VALUES ('1703577335556562945', '6', '8', NULL);
INSERT INTO `le_permission` VALUES ('1703577335564951553', '6', '10', NULL);
INSERT INTO `le_permission` VALUES ('1703577335573340162', '6', '11', NULL);
INSERT INTO `le_permission` VALUES ('1703577335581728770', '6', '12', NULL);
INSERT INTO `le_permission` VALUES ('1703577371011014657', '8', '1', NULL);
INSERT INTO `le_permission` VALUES ('1703577371019403265', '8', '2', NULL);
INSERT INTO `le_permission` VALUES ('1703577371019403266', '8', '3', NULL);
INSERT INTO `le_permission` VALUES ('1703577371027791874', '8', '4', NULL);
INSERT INTO `le_permission` VALUES ('1703577371036180482', '8', '5', NULL);
INSERT INTO `le_permission` VALUES ('1703577371040374785', '8', '6', NULL);
INSERT INTO `le_permission` VALUES ('1703577371040374786', '8', '7', NULL);
INSERT INTO `le_permission` VALUES ('1703577371048763393', '8', '8', NULL);
INSERT INTO `le_permission` VALUES ('1703577371057152002', '8', '9', NULL);
INSERT INTO `le_permission` VALUES ('1703577371065540609', '8', '10', NULL);
INSERT INTO `le_permission` VALUES ('1703577371065540610', '8', '11', NULL);
INSERT INTO `le_permission` VALUES ('1703577371073929218', '8', '12', NULL);
INSERT INTO `le_permission` VALUES ('1703577371082317825', '8', '13', NULL);

-- ----------------------------
-- Table structure for le_role
-- ----------------------------
DROP TABLE IF EXISTS `le_role`;
CREATE TABLE `le_role`  (
  `id` varchar(32) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL,
  `role_name` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,
  `role_code` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,
  `description` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,
  `create_time` datetime NULL DEFAULT NULL,
  `update_time` datetime NULL DEFAULT NULL,
  `status` char(1) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL,
  `parent_id` bigint NULL DEFAULT NULL,
  PRIMARY KEY (`id`) USING BTREE,
  UNIQUE INDEX `unique_role_name`(`role_name` ASC) USING BTREE,
  UNIQUE INDEX `unique_role_value`(`role_code` ASC) USING BTREE
) ENGINE = InnoDB CHARACTER SET = utf8 COLLATE = utf8_general_ci ROW_FORMAT = DYNAMIC;

-- ----------------------------
-- Records of le_role
-- ----------------------------
INSERT INTO `le_role` VALUES ('17', '学生', 'student', NULL, '2023-08-25 08:32:57', '2023-08-25 08:32:57', '1', NULL);
INSERT INTO `le_role` VALUES ('18', '老师', 'teacher', NULL, '2023-08-25 08:32:57', '2023-08-25 08:32:57', '1', NULL);
INSERT INTO `le_role` VALUES ('20', '教学管理员', 'teachmanager', NULL, '2023-08-25 08:32:57', '2023-08-25 08:32:57', '1', NULL);
INSERT INTO `le_role` VALUES ('6', '管理员', 'admin', NULL, '2023-08-25 08:32:57', '2023-08-25 08:32:57', '1', NULL);
INSERT INTO `le_role` VALUES ('8', '超级管理员', 'super', NULL, '2023-08-25 08:32:57', '2023-08-25 08:32:57', '1', NULL);

-- ----------------------------
-- Table structure for le_teacher
-- ----------------------------
DROP TABLE IF EXISTS `le_teacher`;
CREATE TABLE `le_teacher`  (
  `id` varchar(32) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL,
  `user_id` varchar(32) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL COMMENT '用户id',
  `name` varchar(64) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL COMMENT '称呼',
  `intro` varchar(512) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '个人简介',
  `resume` varchar(1024) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '个人简历',
  `pic` varchar(128) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '老师照片',
  PRIMARY KEY (`id`) USING BTREE,
  UNIQUE INDEX `xu_teacher_user_id`(`user_id` ASC) USING BTREE,
  CONSTRAINT `fk_xc_teacher_user_id` FOREIGN KEY (`user_id`) REFERENCES `le_user` (`id`) ON DELETE RESTRICT ON UPDATE RESTRICT
) ENGINE = InnoDB CHARACTER SET = utf8 COLLATE = utf8_general_ci ROW_FORMAT = DYNAMIC;

-- ----------------------------
-- Records of le_teacher
-- ----------------------------

-- ----------------------------
-- Table structure for le_user
-- ----------------------------
DROP TABLE IF EXISTS `le_user`;
CREATE TABLE `le_user`  (
  `id` varchar(64) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL,
  `username` varchar(45) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL,
  `password` varchar(96) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,
  `salt` varchar(45) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,
  `wx_unionid` varchar(128) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '微信unionid',
  `nickname` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '昵称',
  `name` varchar(45) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL,
  `userpic` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '头像',
  `company_id` varchar(32) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,
  `utype` varchar(32) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL,
  `birthday` datetime NULL DEFAULT NULL,
  `sex` char(1) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,
  `email` varchar(45) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,
  `cellphone` varchar(45) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,
  `qq` varchar(32) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,
  `status` varchar(32) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL COMMENT '用户状态',
  `create_time` datetime NOT NULL,
  `update_time` datetime NULL DEFAULT NULL,
  `parent_id` int NULL DEFAULT NULL,
  `path` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,
  `level` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,
  `role_id` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,
  PRIMARY KEY (`id`) USING BTREE,
  UNIQUE INDEX `unique_user_username`(`username` ASC) USING BTREE
) ENGINE = InnoDB CHARACTER SET = utf8 COLLATE = utf8_general_ci ROW_FORMAT = DYNAMIC;

-- ----------------------------
-- Records of le_user
-- ----------------------------
INSERT INTO `le_user` VALUES ('17b9d8be-cef3-400c-bec5-1478a6ef7ddc', 'o5PB1sytMXJGGv2x5OBP2uMzCpS8', 'o5PB1sytMXJGGv2x5OBP2uMzCpS8', NULL, 'o5PB1sytMXJGGv2x5OBP2uMzCpS8', 'Mr.M', 'Mr.M', 'https://thirdwx.qlogo.cn/mmopen/vi_32/P8h3wxWDqCqfI0BmHxAYHEzppsaBp4xt7nscyiaQ2ZptEhzyKcEia7loT6pO6zExaM9FR7BnSguxqKYuIe5B3aEA/132', NULL, '101001', NULL, NULL, NULL, NULL, NULL, '1', '2023-08-21 09:08:51', NULL, NULL, NULL, NULL, NULL);
INSERT INTO `le_user` VALUES ('46', 'super', '$2a$10$TJ4TmCdK.X4wv/tCqHW14.w70U3CC33CeVncD3SLmyMXMknstqKRe', NULL, NULL, NULL, '超级管理员', NULL, '1232141425', '101003', NULL, '1', NULL, NULL, NULL, '1', '2023-08-21 08:33:59', NULL, 1, '0', '0', NULL);
INSERT INTO `le_user` VALUES ('48', 'admin', '$2a$10$TJ4TmCdK.X4wv/tCqHW14.w70U3CC33CeVncD3SLmyMXMknstqKRe', NULL, NULL, NULL, '系统管理员', NULL, '1232141425', '101003', NULL, '1', NULL, NULL, NULL, '1', '2023-08-21 08:34:13', NULL, 1, '1-', '1', NULL);
INSERT INTO `le_user` VALUES ('49', 'it', '$2a$10$TJ4TmCdK.X4wv/tCqHW14.w70U3CC33CeVncD3SLmyMXMknstqKRe', NULL, NULL, NULL, 'test02', NULL, '1232141425', '101002', NULL, '1', '', '12345', NULL, '1', '2023-08-21 08:29:03', NULL, 1, '1-', '1', NULL);
INSERT INTO `le_user` VALUES ('50', 'stu1', '$2a$10$0pt7WlfTbnPDTcWtp/.2Mu5CTXvohnNQhR628qq4RoKSc0dGAdEgm', NULL, NULL, '大水牛', '学生1', 'http://file.xuecheng-plus.com/dddf', '1232141425', '101001', '2023-08-21 09:28:46', '1', NULL, NULL, NULL, '1', '2023-08-21 10:48:01', NULL, 1, '1-', '1', NULL);
INSERT INTO `le_user` VALUES ('51', 'Lucy', '$2a$10$lb1lY/J2hPYuIWeEkeDMm.dSvcdI4D2raikdlcxZNwRy4NHSRznEO', NULL, NULL, NULL, '学生2', NULL, '1232141425', '101001', NULL, '1', NULL, NULL, NULL, '1', '2023-08-21 09:16:11', NULL, 1, '1-', '1', NULL);
INSERT INTO `le_user` VALUES ('52', 'Kyle', '$2a$10$lb1lY/J2hPYuIWeEkeDMm.dSvcdI4D2raikdlcxZNwRy4NHSRznEO', NULL, NULL, NULL, 'Kiki', NULL, '1232141425', '101002', NULL, '1', '6385296@qq.com', NULL, NULL, '', '2023-08-21 09:53:57', NULL, 1, '1-', '1', NULL);
INSERT INTO `le_user` VALUES ('b5dcefda-d942-407b-ac4d-fc9c4b3c1b97', 'oWgGz1FO1I4wzgBGTMP6BOvdk1EY', 'oWgGz1FO1I4wzgBGTMP6BOvdk1EY', NULL, 'oWgGz1FO1I4wzgBGTMP6BOvdk1EY', '时崎狂三', '时崎狂三', 'https://thirdwx.qlogo.cn/mmopen/vi_32/PiajxSqBRaEJbiaadgEJpDPx2GenPaUDngyYoyvibJDlEibsOeiauGr4kftXaSrssLmicgEhmQjdjQEhZHWoEib6WAQsg/132', '1232141425', '101001', NULL, NULL, NULL, NULL, NULL, '1', '2023-08-21 08:52:33', NULL, 1, '1-', '1', NULL);
INSERT INTO `le_user` VALUES ('c94abf17-ecb0-46ed-b022-a9893906f3fe', 'o5PB1s3wYZgJju-nDBGFR0PXu4h8', 'o5PB1s3wYZgJju-nDBGFR0PXu4h8', NULL, 'o5PB1s3wYZgJju-nDBGFR0PXu4h8', 'Blue', 'Blue', NULL, '1232141425', '101001', NULL, NULL, NULL, NULL, NULL, '1', '2023-08-25 10:40:51', NULL, 1, '1-', '1', NULL);
INSERT INTO `le_user` VALUES ('cab93878-c357-4d40-ac07-79d19b8f7232', 'oWgGz1L5soWx0gK7xemSMDQEU8ps', 'oWgGz1L5soWx0gK7xemSMDQEU8ps', NULL, 'oWgGz1L5soWx0gK7xemSMDQEU8ps', 'ㅤ', 'ㅤ', 'https://thirdwx.qlogo.cn/mmopen/vi_32/uVpKJX5jQEicOHDibRtmzTILSCdmU6sWcLMD0DYlZylvibpRfZSSZm1ASDEqxeCHFM6ZWdZ3TkY1WsW8rmRjKIibBA/132', '1232141425', '101001', NULL, NULL, NULL, NULL, NULL, '1', '2023-08-21 08:08:14', NULL, 1, '1-', '1', NULL);
INSERT INTO `le_user` VALUES ('dbcd12d9-2231-4a8b-ad10-6a73b326c334', '123132', '$2a$10$O43pyUc3ytdmHo68BdxjluDw2IlrTB0O2X50Dh0a6h/xpu5yerUCO', NULL, NULL, '123132', '123132', NULL, '1232141425', '101001', NULL, NULL, '1586385296@qq.com', '1586385296@qq.com', NULL, '1', '2023-08-21 08:39:00', NULL, 1, '1-', '1', NULL);

-- ----------------------------
-- Table structure for le_user_role
-- ----------------------------
DROP TABLE IF EXISTS `le_user_role`;
CREATE TABLE `le_user_role`  (
  `id` varchar(64) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL,
  `user_id` varchar(64) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,
  `role_id` varchar(32) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,
  `create_time` datetime NULL DEFAULT NULL,
  `creator` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,
  PRIMARY KEY (`id`) USING BTREE,
  INDEX `fk_xc_user_role_user_id`(`user_id` ASC) USING BTREE,
  INDEX `fk_xc_user_role_role_id`(`role_id` ASC) USING BTREE
) ENGINE = InnoDB CHARACTER SET = utf8 COLLATE = utf8_general_ci ROW_FORMAT = DYNAMIC;

-- ----------------------------
-- Records of le_user_role
-- ----------------------------
INSERT INTO `le_user_role` VALUES ('1', '46', '8', '2023-08-15 14:03:12', '超级管理员');
INSERT INTO `le_user_role` VALUES ('2', '48', '6', '2023-08-17 09:16:27', '超级管理员');
INSERT INTO `le_user_role` VALUES ('20', '50', '17', '2023-08-17 13:03:12', '');
INSERT INTO `le_user_role` VALUES ('2034706c-7d15-43fb-8b0d-bd581c7ace02', 'e63fb882-3226-4365-9711-d3fd467a4813', '17', NULL, NULL);
INSERT INTO `le_user_role` VALUES ('21', '52', '20', NULL, NULL);
INSERT INTO `le_user_role` VALUES ('3', '49', '18', '2023-08-17 13:11:20', NULL);
INSERT INTO `le_user_role` VALUES ('48d759c1-a597-4220-8ddf-91696638d91d', '17b9d8be-cef3-400c-bec5-1478a6ef7ddc', '17', NULL, NULL);
INSERT INTO `le_user_role` VALUES ('88f646ec-7257-4987-816e-92adfb68e0c1', 'e47ade0d-377d-4e58-99fc-6d5e2af5c148', '17', NULL, NULL);
INSERT INTO `le_user_role` VALUES ('b5dcefda-d942-407b-ac4d-fc9c4b3c1b97', 'b5dcefda-d942-407b-ac4d-fc9c4b3c1b97', '17', '2023-08-21 11:02:33', NULL);
INSERT INTO `le_user_role` VALUES ('c94abf17-ecb0-46ed-b022-a9893906f3fe', 'c94abf17-ecb0-46ed-b022-a9893906f3fe', '17', NULL, NULL);
INSERT INTO `le_user_role` VALUES ('cab93878-c357-4d40-ac07-79d19b8f7232', 'cab93878-c357-4d40-ac07-79d19b8f7232', '17', '2023-08-22 09:08:14', NULL);
INSERT INTO `le_user_role` VALUES ('cf0f944a-d22b-40a9-a107-d74a27733604', 'c94abf17-ecb0-46ed-b022-a9893906f3fe', '17', '2023-08-25 10:40:51', NULL);
INSERT INTO `le_user_role` VALUES ('dbcd12d9-2231-4a8b-ad10-6a73b326c334', 'dbcd12d9-2231-4a8b-ad10-6a73b326c334', '17', '2023-08-25 14:39:00', NULL);

-- ----------------------------
-- Table structure for oauth_access_token
-- ----------------------------
DROP TABLE IF EXISTS `oauth_access_token`;
CREATE TABLE `oauth_access_token`  (
  `token_id` varchar(256) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,
  `token` blob NULL,
  `authentication_id` varchar(48) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL,
  `user_name` varchar(256) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,
  `client_id` varchar(256) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,
  `authentication` blob NULL,
  `refresh_token` varchar(256) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,
  PRIMARY KEY (`authentication_id`) USING BTREE
) ENGINE = InnoDB CHARACTER SET = utf8 COLLATE = utf8_general_ci ROW_FORMAT = DYNAMIC;

-- ----------------------------
-- Records of oauth_access_token
-- ----------------------------

-- ----------------------------
-- Table structure for oauth_approvals
-- ----------------------------
DROP TABLE IF EXISTS `oauth_approvals`;
CREATE TABLE `oauth_approvals`  (
  `userId` varchar(256) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,
  `clientId` varchar(256) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,
  `scope` varchar(256) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,
  `status` varchar(10) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,
  `expiresAt` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `lastModifiedAt` timestamp NOT NULL
) ENGINE = InnoDB CHARACTER SET = utf8 COLLATE = utf8_general_ci ROW_FORMAT = DYNAMIC;

-- ----------------------------
-- Records of oauth_approvals
-- ----------------------------

-- ----------------------------
-- Table structure for oauth_client_details
-- ----------------------------
DROP TABLE IF EXISTS `oauth_client_details`;
CREATE TABLE `oauth_client_details`  (
  `client_id` varchar(48) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL,
  `resource_ids` varchar(256) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,
  `client_secret` varchar(256) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,
  `scope` varchar(256) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,
  `authorized_grant_types` varchar(256) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,
  `web_server_redirect_uri` varchar(256) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,
  `authorities` varchar(256) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,
  `access_token_validity` int NULL DEFAULT NULL,
  `refresh_token_validity` int NULL DEFAULT NULL,
  `additional_information` varchar(4096) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,
  `autoapprove` varchar(256) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,
  PRIMARY KEY (`client_id`) USING BTREE
) ENGINE = InnoDB CHARACTER SET = utf8 COLLATE = utf8_general_ci ROW_FORMAT = DYNAMIC;

-- ----------------------------
-- Records of oauth_client_details
-- ----------------------------
INSERT INTO `oauth_client_details` VALUES ('app', NULL, 'app', 'app', 'password,refresh_token', NULL, NULL, NULL, NULL, NULL, NULL);
INSERT INTO `oauth_client_details` VALUES ('LeWebApp', NULL, 'LeWebApp', 'app', 'authorization_code,password,refresh_token,client_credentials', NULL, NULL, 43200, 43200, NULL, NULL);

-- ----------------------------
-- Table structure for oauth_client_token
-- ----------------------------
DROP TABLE IF EXISTS `oauth_client_token`;
CREATE TABLE `oauth_client_token`  (
  `token_id` varchar(256) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,
  `token` blob NULL,
  `authentication_id` varchar(48) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL,
  `user_name` varchar(256) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,
  `client_id` varchar(256) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,
  PRIMARY KEY (`authentication_id`) USING BTREE
) ENGINE = InnoDB CHARACTER SET = utf8 COLLATE = utf8_general_ci ROW_FORMAT = DYNAMIC;

-- ----------------------------
-- Records of oauth_client_token
-- ----------------------------

-- ----------------------------
-- Table structure for oauth_code
-- ----------------------------
DROP TABLE IF EXISTS `oauth_code`;
CREATE TABLE `oauth_code`  (
  `code` varchar(256) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,
  `authentication` blob NULL
) ENGINE = InnoDB CHARACTER SET = utf8 COLLATE = utf8_general_ci ROW_FORMAT = DYNAMIC;

-- ----------------------------
-- Records of oauth_code
-- ----------------------------

-- ----------------------------
-- Table structure for oauth_refresh_token
-- ----------------------------
DROP TABLE IF EXISTS `oauth_refresh_token`;
CREATE TABLE `oauth_refresh_token`  (
  `token_id` varchar(256) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,
  `token` blob NULL,
  `authentication` blob NULL
) ENGINE = InnoDB CHARACTER SET = utf8 COLLATE = utf8_general_ci ROW_FORMAT = DYNAMIC;

-- ----------------------------
-- Records of oauth_refresh_token
-- ----------------------------
INSERT INTO `oauth_refresh_token` VALUES ('b96e057b4e1c4920428e833db48d4c15', , );

SET FOREIGN_KEY_CHECKS = 1;
